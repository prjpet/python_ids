...
import os, sys, pyshark, time, hashlib
...
def getState(packet_contents,device_list, i):
    ...
    final_dict = {}
    print("Packet: ", i, "********** READING ALL DEVICES WITH FUNCTION CODE "+str(func_code)+" - REGISTER DEFAULT. **********")
    if func_code == 3:
        ...
        for device_value in registers:
            ...
            if device_index in device_list:
                device_list[device_index].default_state = device_value
                    ...
                    if not request:
                        ...
                        if mode_index == 0:
                            ...
                            if next_packet_time - first_packet_time > mode[mode_index][0]:
                                ...
                                block_time = 0
                                prev_block_time = 0
                            #and mySystemState.next_state[1]["digital"] != mySystemState.default_state[1]["digital"]: mode_index += 1
                            ...
                            #we are still in learning phase:
                                ...
                                if packet_contents["func_code"] == 3:
                                    ...
                                    if mySystemState.default_state == 0:
                                        mySystemState.default_state = getState(packet_contents,modbus_device_list, i)
                                        mySystemState.next_state = mySystemState.default_state
                                    else:
                                        mySystemState.next_state = getState(packet_contents,modbus_device_list, i)

                                #printDevices("Default state: ", mySystemState.default_state)
                            ...
                            if packet_contents["func_code"] == 3:
                                print("********* LEARNING DEFAULTS FINISHED - HASHING DIGITAL STATE.**********")
                                ...
                                for logical_block in current_system_state:
                                    ...
                                    if hashed_block in mySystemState.digital_statechart[logical_block]:
                                        ...
                                        #else:
                                            #mySystemState.digital_statechart[logical_block][hashed_block]["successors"].append(hashed_block)
                                            #item_index = mySystemState.digital_statechart[logical_block][hashed_block]["successors"].index(hashed_block)
                                            #mySystemState.digital_statechart[logical_block][hashed_block]["prob"][item_index] += 1
                                    ...
                                    if previous_system_state != 0:
                                        if hashed_block not in mySystemState.digital_statechart[logical_block][prev_hashed_block]["successors"]:
                                            ...
                                            mySystemState.digital_statechart[logical_block][prev_hashed_block]["time_delta"].append(block_time - prev_block_time)
                                            ...
                                            mySystemState.digital_statechart[logical_block][prev_hashed_block]["time_delta"][item_index] = block_time - prev_block_time
